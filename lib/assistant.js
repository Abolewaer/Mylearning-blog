...
(async () => {
  ...
  mount.innerHTML = `<button class="assistant-launcher" type="button" aria-expanded="false" aria-controls="assistant-panel"><span class="assistant-ghost" aria-hidden="true"></span><span>笔记助手</span></button>
    ...
    <div class="assistant-controls"><select aria-label="使用身份"><option value="guest">游客模式</option><option value="owner">作者模式</option></select><button type="button" class="assistant-login">验证作者</button><a class="assistant-write" hidden>写作 ↗</a></div>
    <form class="assistant-auth" hidden><label>作者密码<input type="password" name="pin" autocomplete="current-password" required maxlength="128"></label><button type="submit">确认</button></form>
  ...
  $('.assistant-auth').onsubmit = async event => {
    ...
    try {
      const r = await fetch(config.endpoint + '/owner/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ pin }) });
      const result = await r.json();
      if (!r.ok) throw new Error(result.error || '验证失败');
      ownerToken = result.token;
      pinInput.value = ''; $('.assistant-auth').hidden = true;
      mode.value = 'owner'; $('.assistant-write').hidden = false; mode.onchange();
  ...
  $('.assistant-compose').onsubmit = async event => {
    ...
    try {
      ...
      const result = await r.json();
      if (!r.ok) { if (r.status === 401) { ownerToken = ''; $('.assistant-write').hidden = true; } throw new Error(result.error || '请求失败'); }
